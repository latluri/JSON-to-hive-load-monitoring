...
if [ $status != 0 ];
then
    echo "Failed at row counts before loading lacek_comarch_raw_clm "|mailx -s "Load_failure_lacek_comarch_raw_clm" latluri1@ford.com
    exit 1
fi
...
if [ $status != 0 ];
then
    echo "Failed at dropping the existing tables lacek_comarch_raw_clm "|mailx -s "Load_failure_lacek_comarch_raw_clm" latluri1@ford.com
    exit 1
fi
...
if [ $status != 0 ];
then
    echo "Failed at loading data into lacek_comarch_raw_clm "|mailx -s "Load_failure_lacek_comarch_raw_clm" latluri1@ford.com
    exit 1
fi
...
if [ $status != 0 ];
then
    echo "Failed at row counts after loading lacek_comarch_raw_clm "|mailx -s "Load_failure_lacek_comarch_raw_clm" latluri1@ford.com
    exit 1
fi

join -1 1 -2 1 b4${date}.txt after${date}.txt| perl -p -e 's/^pipe/\npipe/g' |  mailx -s Table\ Counts\ Comarch\ Raw\ CLM  -a  after${date}.txt  latluri1@ford.com
