...
def update_df(from_df,dump_df,deciding_col_rename,reason,list_col_bad_rec,deciding_col):
    if deciding_col in from_df.columns:
        ...
        if dump_temp.count()>0:
            print(reason+": "+str(dump_temp.count()))
...
def  add_columns(final,table):
    ...
    if len(list_2_add_maintain_schema)>0:
        print(table+" JSON contains less columns than expected such as: "+str(list_2_add_maintain_schema).replace("set([","").replace("])",""))
        for i386 in list_2_add_maintain_schema:
            final=final.withColumn(i386,lit(unicode("")))
    if len(list_2_add)>0:
        print(table+"______________________New column(s) added "+str(list_2_add).replace("set([","").replace("])",""))
        updated_col_table=col_in_table+list(list_2_add)
        final=final.select(*updated_col_table)
    else:
        print(table+"______________________No new columns found, adding new rows to the table")
...
today_full=datetime.today().strftime('%Y-%m-%d %H:%M:%S')
df=sqlContext.read.json("JSON_INPUT_init").cache()
Raw_jsons=df.count()
...
################################### prints "duplicates within the batch all the time"    
        final,duplicate_records=update_df(from_df=final,dump_df=duplicate_records,deciding_col_rename="temp_col",reason="Duplicates within the batch",list_col_bad_rec=["entitystring","process__id","gdia_load_date","reason","sha_key"],deciding_col="dupeCount")
...
############################################################################################################################
            ...
            final,duplicate_records=update_df(from_df=final,dump_df=duplicate_records,deciding_col_rename="temp_col",reason="Duplicates accross the table",list_col_bad_rec=["entitystring","process__id","gdia_load_date","reason","sha_key"],deciding_col="sha_key_2load")
############################################################################################################################
...
############################################################################################################################
            ...
            final=final.drop('entitystring')
            print(str(i344)+":"+str(final.count()))
            hive_create_new_table(final,db_location,db,i344)
    else:
        print("No Data to load the table "+i344+". Please Investigate for ALL bad records")



if bad_records.count()>0:
    if "bad_records" in tables_in_db:
        print("bad_records :"+str(bad_records.count()))
...
if duplicate_records.count()>0:
    if "duplicate_records" in tables_in_db:
        print("duplicate_records :"+str(duplicate_records.count()))
