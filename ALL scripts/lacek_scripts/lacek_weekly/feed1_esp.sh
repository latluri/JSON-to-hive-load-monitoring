...
#--delete-target-dir 
###################################
# ALTER TABLE lacek_fpr.pipe_del_esp_table SET TBLPROPERTIES('EXTERNAL'='TRUE');
...
--target-dir  hdfs://hdp2cluster/project/dz/collab/LACEK/lacek_fpr/pipe_del_esp_table \


#count after
after=`beeline --showHeader=false --outputformat=csv2 -e "select count(*) from lacek_fpr.pipe_del_esp_table"`

after2=`hdfs dfs -cat /project/dz/collab/LACEK/lacek_fpr/pipe_del_esp_table/*|wc -l`

if [ $after != $after2 ];
then
    echo "Please verify the record counts for table pipe_del_esp_table"|mailx -s "Please verify the record counts for table pipe_del_esp_table" latluri1@ford.com
fi


echo -e "Counts b4 loading: ${b4} \nCounts after loading ${after}"|mailx -s "Lacek_loading_report_feed1_pipe_del_esp_table" latluri1@ford.com
