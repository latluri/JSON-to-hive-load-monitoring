...
beeline --silent=true  --showHeader=false --outputformat=tsv2 -e "use dsc60263_fsm_tz_db; select data from bad_record where message__id is null and to_date(lastupdatedatetime)  = date_sub(CAST(current_timestamp() as DATE), 1)" >  ${yesterday_1}_bad_rec_null_msg_id.txt
