...
for k in tables_gcct_except_bad_rec:
            v1=sqlContext.table(k).filter("to_date(lastupdatedatetime)  =  date_sub(CAST(current_timestamp() as DATE), 0)")
            g1_1=v1.count()
            g1=g1+g1_1
            v3={}
...
g3=sqlContext.table('bad_record').filter("to_date(lastupdatedatetime)  =  date_sub(CAST(current_timestamp() as DATE), 0) and exception like '%Datatype discrepancy%'").groupby("datatype").count().sort("datatype")
...
g4=sqlContext.table('bad_record').filter("to_date(lastupdatedatetime)  =  date_sub(CAST(current_timestamp() as DATE), 0) and exception like '%not part of approved entities%'").groupby("datatype").count().sort("datatype")
...
g5=sqlContext.table("bad_record").filter("to_date(lastupdatedatetime)  =  date_sub(CAST(current_timestamp() as DATE), 0)").filter("reason like '%Duplicate%' ").count()
g6=sqlContext.table("bad_record").filter("to_date(lastupdatedatetime)  =  date_sub(CAST(current_timestamp() as DATE), 0)").filter("exception = 'Empty Entity Data'").count()
g2=sqlContext.table("bad_record").filter("to_date(lastupdatedatetime)  =  date_sub(CAST(current_timestamp() as DATE), 0)").count()
